        ...
        <div
            ...
            <div
                ...
                <h4 class="text-xl font-bold text-gray-800 dark:text-gray-100 flex items-center gap-3">
                    ...
                    <span class="text-lg sm:text-xl">Roles and Permissions List</span>
                </h4>
            ...
            <div class="flex-1 overflow-y-auto px-3 py-3 sm:px-5 sm:py-3 space-y-4 sm:space-y-2">
                ...
                <div class="overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-700 shadow-lg">
                    <DataTablePcBody :items="apiData?.results || []" :columns="tableColumns"
                        ...
                        <!-- Total Permission -->
                        <template #cell-total_permission="{ row }">
                            <div class="text-sm text-gray-600 dark:text-gray-300">{{ row.total_permissions || 0 }}</div>
                        </template>
...
<script setup>
import { RoleApiURL, RolePageURL } from '../../routes';
...
const tableColumns = [
    { field: 'name', title: 'Name', width: '40%', sticky: true, sortable: true },
    { field: 'total_permission', title: 'Total Permission', width: '40%', sticky: true, sortable: false },
]
...
const fetchData = async () => {
    ...
    try {
        ...
        const response = await axios.get(RoleApiURL.List, { params });
...
const addItem = () => {
    ...
    window.location = RolePageURL.Create;
};
const editItem = (item) => {
    // console.log('Edit item:', item);
    window.location = `${RolePageURL.Update}/${item.id}`;
};

const viewItem = (item) => {
    console.log('View item:', item.id);
    window.location = `${RolePageURL.Details}/${item.id}/`;
};

const handleDelete = (id) => {
    deleteItem({
        url: `${RoleApiURL.Delete}`,
        id: id,
        name: 'Role',
        onSuccess: fetchData,
    });
