   ...
   <MasterCardLayout :is-loading="loadingStates.loading">
      ...
      <MainContentCard :error="error">
         ...
         <template #body>
            ...
            <!-- ========= [ ADD MODAL ] ============ -->
            <CustomModal :isOpen="showAddItemModal" @update:isOpen="showAddItemModal = $event" title="Add New Color"
               size="sm">
               <template #body>
                  <form @submit.prevent="handleAddItem" class="space-y-5">
                     <div class="space-y-3">
                        <!-- Name -->
                        <InputeComponent label="Color Name" id="name" name="name" label-for="name"
                           placeholder="Enter Color Name" v-model="addForm.name" :error="addFormErrors.name" />
            ...
            <!-- ========= [ EDIT MODAL ] ============ -->
            <CustomModal :isOpen="showEditItemModal" @update:isOpen="showEditItemModal = $event" title="Edit Color"
               size="sm">
               <template #body>
                  <form @submit.prevent="handleUpdateItem" class="space-y-5">
                     <div class="space-y-3">
                        <!-- Name -->
                        <InputeComponent label="Color Name" id="edit_name" name="edit_name" label-for="edit_name"
                           placeholder="Enter Color Name" v-model="editForm.name" :error="editFormErrors.name" />
...
<script setup>
import { ColorApiURL } from '../../routes';
...
const fetchData = async () => {
   ...
   try {
      ...
      const response = await axios.get(`${ColorApiURL.List}/`, { params });
...
const handleUpdateItem = async () => {
   ...
   try {
      const response = await axios.put(
         `${ColorApiURL.Update}/${editForm.value.id}/`,
         formDataToSend,
         {}
      );
